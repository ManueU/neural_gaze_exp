    ...
    for array = 1:n_arrays
        ...
        for channel = 1:n_channels
            ...
            for target = 1:n_targets
                ...
                ZS_smooth = smoothdata(zscored, 1, 'gaussian', w);
...
% Scelta robusta delle componenti:
% prendo le componenti con maggiore varianza spiegata
% dentro ciascuna marginalizzazione

compVar = explVar.componentVar(:);   % [nComponents x 1]

% --- Goal / Target components ---
if numel(tgtIDs) >= 2
    [~, ordTgt] = sort(compVar(tgtIDs), 'descend');
    kGoal1 = tgtIDs(ordTgt(1));
    kGoal2 = tgtIDs(ordTgt(2));
else
    warning('Meno di 2 componenti target-like: uso fallback basato su dirScore.');
    kGoal1 = dirOrder(1);
    kGoal2 = dirOrder(2);
end

% --- Condition / Gaze component ---
if ~isempty(condIDs)
    [~, ordCond] = sort(compVar(condIDs), 'descend');
    kCondition = condIDs(ordCond(1));
else
    warning('Nessuna componente condition-like: uso fallback basato su condScore.');
    kCondition = condOrder(1);
end

fprintf('\nComponenti selezionate per plot 3D:\n');
fprintf('Goal Dim 1      = dPC %d | marg = %d | var = %.4f\n', ...
    kGoal1, whichMarg(kGoal1), compVar(kGoal1));
fprintf('Goal Dim 2      = dPC %d | marg = %d | var = %.4f\n', ...
    kGoal2, whichMarg(kGoal2), compVar(kGoal2));
fprintf('Condition Dim 1 = dPC %d | marg = %d | var = %.4f\n', ...
    kCondition, whichMarg(kCondition), compVar(kCondition));
