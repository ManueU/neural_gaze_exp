...
% d = 1; 
load(filename);
visual = readtable(filename_excel);
...
%% Stabilisco se l'elettrodo è modulante o meno
...
for array = 1:n_arrays
    for channel = 1:n_channels
        ...
        for target = 1:n_targets
            ...
            mu_baseline  = mean_baseline.by_targets{1,d}(target, ch_global);
            sig_baseline = std_baseline.by_targets{1,d}(target, ch_global);
            data_z = (mean_across_trials - mu_baseline) ./ sig_baseline;
            
            if visual.PRE(ch_global) == 0 && visual.REACH(ch_global) == 0
               modulation(ch_global, target) = 0;
            else 
